...
### TaiKhoanController
- `login(tenDangNhap, matKhau)` — verify bcrypt, return taiKhoan hoặc error code
- `logout(maTaiKhoan)` — no-op (stateless auth)
- `checkRole(maTaiKhoan, vaiTro)` — check user có role không
- CRUD tài khoản (admin only): `listAccounts`, `createAccount`, `updateStatus`, `resetPassword`, `deleteAccount`
...
### SachController
- `listBooks()` — trả về `SachWithAvailability[]` (có soDangMuon, soKhaDung)
- `getBookById(maSach)` — 1 sách với availability
- `searchBooks(keyword, onlyAvailable?)` — diacritics-aware
- `createBook`, `updateBook`, `deleteBook` — CRUD
- `updateBook` validate: `soBanSao >= soMat + soBaoTri + soDangMuon`
- `getAvailableCount(maSach)`, `getActiveLoanCount(maSach)` — helper
- `incrementLost(maSach)` — tăng soMat khi trả + đánh dấu mất

### PhieuMuonController
**Quan trọng nhất** — chứa 3 use case chính:
- `getActiveLoans()`, `searchActiveLoans(keyword, searchType)` — list phiếu đang mượn (diacritics-aware)
- `validateMember(maDocGia)` — check thẻ tồn tại + chưa hết hạn
- `checkBookAvailability(maSach)` — check `soKhaDung > 0`
- `createLoan(maDocGia, maSach)` — tạo phiếu (transaction, không update Sach)
- `calculateFine(hanTra, ngayTra)` — công thức tính phạt
- `returnBook(maPhieu, { daMatSach?, phiMat? })` — trả sách; nếu mất, tăng soMat + charge phiMat
- `extendLoan(maPhieu)` — gia hạn +7 ngày
- `getLoanDetails(maPhieu)` — full info + reader + book (cho in phiếu)
...
```bash
cd backend && LOG_LEVEL=debug pnpm run dev    # Linux/Mac
cd backend && set LOG_LEVEL=debug && pnpm run dev    # Windows CMD
```

Hoặc dùng script sẵn: `pnpm run dev:debug`, `pnpm run dev:quiet`.
...
## 4.8. Diacritics search (shared utils)

File `backend/utils/diacritics.ts` xuất 4 hàm:

```ts
// Core: bỏ dấu
removeDiacritics(str) → string

// So sánh 1 field với keyword (case + diacritics insensitive)
matchesKeyword(field, keyword) → boolean

// OR qua nhiều field
matchesAny(fields[], keyword) → boolean

// Filter danh sách với keyword + hàm lấy field
filterByKeyword(records, keyword, getFields) → T[]
```
...
searchBooks(keyword, onlyAvailable?) {
  const all = this.listBooks();
  let filtered = filterByKeyword(all, keyword, b => [b.maSach, b.tieuDe, b.tacGia]);
  if (onlyAvailable) filtered = filtered.filter(b => b.soKhaDung > 0);
  return filtered;
}

// PhieuMuonController.searchActiveLoans (switch theo searchType)
filtered = rows.filter(r => matchesAny([r.tenDocGia, r.tenSach, ...], keyword));
```
...
`backend/tests/` có 8 test file, 97 test cases. Chạy:
